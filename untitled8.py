...
def _download_map_image(min_lat=45.0, min_lon=7.6, max_lat=45.1, max_lon=7.7, size=2000):
    ...
    static_map.add_polygon(Polygon(((min_lon, min_lat), (min_lon, max_lat), (max_lon, max_lat), (max_lon, min_lat)), None, '#FFFFFF'))
...
def _get_coordinates_from_dataset(dataset_folder, extension="jpg"):
    ...
    and returns
        [("train/gallery", [[45, 8.1], [45.2, 8.2]]), ("train/queries", [[45, 8.1], [45.2, 8.2]])]
    """
    images_paths = glob(join(dataset_folder, "**", f"*.{extension}"), recursive=True)
    if len(images_paths) != 0: print(f"I found {len(images_paths)} images in {dataset_folder}")
    else: raise ValueError(f"I found no images in {dataset_folder} !")
...
def build_map_from_dataset(dataset_folder, dot_sizes=None):
    ...
    try:
        ...
        pass  # This dataset has different folder names than the standard train-val-test gallery-queries.
...
# Togliere il test set da MSLS
for dn, s in [
        # ('pitts30k', [40,20,40,20,40,20]),
        ('msls', [200,50,200,50]),
        # ('msls+', [40,20,40,20]),
        # ('tokyo247', [20, 150]),
        # ('san_francisco', [10, 30]), ('eynsham', [20, 5]), ('st_lucia', [20, 5])
              ]:
    print(dn)
    map_img = build_map_from_dataset(dn, s)
    import shutil
    # if "+" in dn:
    #     shutil.copy(f"map_msls_sf.png", f"/home/gabriele/")
    # else:
    shutil.copy(f"map_{dn}.png", f"/home/gabriele/")
